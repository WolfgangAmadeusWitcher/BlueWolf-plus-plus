...
BwppStatus bwpp_codegen_metal(const BwppIrModule *ir, const char *out_path) {
  ...
  if (tile) {
    ...
    if (!has_attention) {
      ...
      fputs("    float scores[BWPP_ATT_TILE_N];\n", f);
      fputs("    for (uint i = 0; i < BWPP_ATT_TILE_N; ++i) { scores[i] = 0.0f; }\n", f);
      ...
      fputs("        float acc = 0.0f;\n", f);
      fputs("        for (uint kk = 0; kk < tile; ++kk) {\n", f);
      fputs("          acc += float(Qtg[tid.y][kk]) * float(Ktg[n][kk]);\n", f);
      fputs("        }\n", f);
      fputs("        scores[n] += acc;\n", f);
      fputs("      }\n", f);
      fputs("      threadgroup_barrier(mem_flags::mem_threadgroup);\n", f);
      ...
      fputs("    for (uint n = 0; n < tile; ++n) {\n", f);
      fputs("      uint idx = n0 + n;\n", f);
      fputs("      if (idx < p.N && scores[n] > maxv) { maxv = scores[n]; }\n", f);
      fputs("    }\n", f);
      fputs("  }\n", f);
      ...
      fputs("      float w = exp(scores[n] - maxv);\n", f);
      fputs("      sum += w;\n", f);
